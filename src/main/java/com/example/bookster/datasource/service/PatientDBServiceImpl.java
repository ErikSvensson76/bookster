...
public class PatientDBServiceImpl implements PatientDBService {
    ...
    public Mono<Void> delete(Mono<UUID> uuidMono) {
       return uuidMono.flatMap(repository::findById)
                .flatMap(dbPatient -> Mono.zip(
                        Mono.from(contactInfoDBService.delete(Mono.just(dbPatient.getContactInfoId()))),
                        Mono.from(appUserDBService.delete(Mono.just(dbPatient.getAppUserId()))))
                        .then(repository.deleteById(dbPatient.getId())));
    }
}
